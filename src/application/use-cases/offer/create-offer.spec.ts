...
describe('Create Offer Use Case', () => {
  ...
  beforeAll(async () => {
    walletCoinRepository.create(
      new WalletCoin({ balance: 2, coinId: 1, walletId: 1 }, 1),
    );

    createOfferUseCase = new CreateOffersUseCase(
      offerRepository,
      walletCoinRepository,
    );
  });

  it('should create an offer', async () => {
    const offer = await createOfferUseCase.execute(
      { quantity: 2, walletCoinId: 1 },
      1,
    );
  ...
  it('should not create if quantity is less than 1', async () => {
    await expect(
      createOfferUseCase.execute({ quantity: 0, walletCoinId: 1 }, 1),
    ).rejects.toThrow(new BadRequestException('Quantity must be positive.'));
  });

  it('should not create if quantity is greater than balance', async () => {
    await expect(
      createOfferUseCase.execute({ quantity: 3, walletCoinId: 1 }, 1),
    ).rejects.toThrow(new BadRequestException('Insufficient balance.'));
  });

  it('should not create if walletCoinId does not exist', async () => {
    await expect(
      createOfferUseCase.execute({ quantity: 1, walletCoinId: 2 }, 1),
    ).rejects.toThrow(new BadRequestException('WalletCoin not found.'));
  });
  ...
  it('should not create if user already create 5 offers on same day', async () => {
    ...
    await expect(
      createOfferUseCase.execute({ quantity: 1, walletCoinId: 1 }, 1),
    ).rejects.toThrow(new BadRequestException('Today offers limit exceeded.'));
  });
});
